...
import { ScrollToTopWrapper } from "@/components/scroll-to-top-wrapper";
import { RequestInformationDialog } from "@/components/property-details/request-information";
import { locationPropertiesMetadata, LocationSlug } from "@/seo-metadata/location-properties";
...
export async function generateMetadata({
  ...
  // ========================================
  if (isLocationPage(propertyId)) {
    const locationConfig = locationPropertiesMetadata[propertyId];
    const metadata = locationConfig[locale as keyof typeof locationConfig] || locationConfig.en;
    
    const canonicalUrl = `${BASE_URL}/${locale}/properties/${propertyId}`;

    // Build alternate language URLs
    const languages: Record<string, string> = {};
    routing.locales.forEach((loc) => {
      languages[loc] = `${BASE_URL}/${loc}/properties/${propertyId}`;
    });
    languages["x-default"] = `${BASE_URL}/${routing.defaultLocale}/properties/${propertyId}`;
  ...
  // ========================================
  // PROPERTY DETAILS PAGE METADATA
  // ========================================
  const propertyResponse = await getProperty(propertyId);
  const property = propertyResponse.data;
...
export default async function DynamicPage(props: PageProps) {
  ...
  // Route to property details page
  return <PropertyDetailsPage {...props} />;
}
...
async function PropertyDetailsPage(props: PageProps) {
  return (
    <div className="py-14">
      <ScrollToTopWrapper>
        <Suspense fallback={<PropertyDetailsPageLoading />}>
...
const PropertyDetailsContent = async (props: PageProps) => {
  const t = await getTranslations("propertyDetailsPage");
  const { propertyId, locale } = await props.params;
  const session = await auth();
  const token = session?.accessToken;
...
// ============================================================================
// SHARED PROPERTIES LIST COMPONENT
// ============================================================================
