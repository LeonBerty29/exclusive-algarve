...
export const ProductCard = ({ property }: Props) => {
  ...
  const imagePaths = property.assets?.images?.gallery?.map(img => img.url) || [];
    ...
    <Card className="flex flex-col gap-0 rounded-none p-0">
      ...
      <CardContent className='p-3 space-y-3'>
        <div className='flex justify-between align-center flex-wrap gap-y-2 gap-x-5'>
          {showPrice ? (
            <PriceFormat amount={property.price} currency={currency} amountStyle="text-primary" formatAmount={false} className="gap-1 text-primary" />
          ) : (
            <span className="text-base font-semibold flex items-center gap-1 text-primary">Contact for price</span>
          )}
        </div>
        ...
        <div className="flex flex-wrap lg:flex-nowrap justify-between gap-2">
          <div className="w-[47%]">
            <div>
              <p className='text-[11px] font-light text-gray-400 mb-3'>Location</p>
              <p className='text-[11px] font-semibold text-black line-clamp-1'>{location}</p>
            </div>
          </div>

          <div className='w-[52%] max-w-[140px] min-w[128px] flex gap-2 justify-between'>
            <div className="flex-1 max-w-fit">
              <p className='text-[11px] font-light text-gray-400 mb-3'>Gross Area</p>
              <p className='text-[11px] text-black font-semibold'>{grossArea} <sup>m²</sup></p>
            </div>
            <div className="flex-1 max-w-fit">
              <p className='text-[11px] font-light text-gray-400 mb-3'>Plot Size</p>
              <p className='text-[11px] font-semibold text-black'>{plotSize} <sup>m²</sup></p>
            </div>
          </div>
        </div>
